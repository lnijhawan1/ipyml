...
def run_neural_net(model: dict, **kwargs):
    inputs = [kwargs[var_name] for var_name in model["inputs"]]
    # Output  and hidden activations
    output_activation = ACTIVATION_FUNCTIONS.get(
        model["output_activation"], lambda z: z
    )
    hidden_activation = ACTIVATION_FUNCTIONS.get(
        model["hidden_activation"], lambda z: z
    )
