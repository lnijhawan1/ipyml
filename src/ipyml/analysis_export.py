...
class FinalModel(ipyw.VBox):
    def __init__(self, trained_model: TrainedModel, *args, **kwargs):
        ...
        text_widgets = [ipyw.HTML(f'{param}: {trained_model.params[param]}') for param in trained_model.params]
        
        # select available plots, this can be appended in the future
        available_plots = ['Predicted vs. Actual', 'Residuals', 'Training vs. Validation Residuals']
        ...
        self.children = [ipyw.HTML('This model was trained with the following parameters:'),] + text_widgets+ [
    ...
    def view_plot(self, *_):
        ...
        X_test_tr = self.scaler.transform(self.trained_model.test_data.drop([target_name], axis=1))
        X_train_tr = self.scaler.transform(self.trained_model.train_data.drop([target_name], axis=1))

        X_test_tr = pd.DataFrame(X_test_tr, index=self.trained_model.test_data.index, columns=self.trained_model.test_data.drop([target_name],axis=1).columns)
        X_train_tr = pd.DataFrame(X_train_tr, index=self.trained_model.train_data.index, columns=self.trained_model.train_data.drop([target_name],axis=1).columns)
        ...
        if self.plot_selector.value == 'Predicted vs. Actual':
            with self.plot_output:
                ...
                plt.scatter(actual_test, predicted_test, color='tab:orange', alpha=.5, label='Test Data')
                plt.scatter(actual_train, predicted_train, color='tab:blue', alpha=.5, label='Train Data')
        ...
        if self.plot_selector.value == 'Residuals':
            with self.plot_output:
                ...
                plt.scatter(predicted_test, test_residuals, color='tab:orange', alpha=.5, label='Test Data')
                plt.scatter(predicted_train, train_residuals, color='tab:blue', alpha=.5, label='Train Data')
                plt.xlabel("Predicted Values")
                plt.ylabel("Residual Values")
                plt.title("Residiual Value Plot")
                plt.legend()
                plt.plot(predicted_test, [0 for i in predicted_test], color='k', lw=1, scalex=False, scaley=False)
                plt.show()
        ...
        if self.plot_selector.value == 'Training vs. Validation Residuals':
            with self.plot_output:
                ...
                train_residuals = predicted_train - actual_train
                test_residuals = predicted_test - actual_test   
                plt.hist(test_residuals, color='tab:orange', alpha=.5, label='Test Data')
                plt.hist(train_residuals, color='tab:blue', alpha=.5, label='Train Data')
    ...
    def export(self, *_):
        ...
        if self.trained_model.model_type == 'statsmodel_OLS':
            ...
            saved_formula = f'{target_col} ~ ' + '+'.join(self.trained_model.model.params.index).replace('Intercept','1') # (is this necessary?)                                                                                                                                                                                                                                                                   
