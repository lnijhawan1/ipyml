...
from .model_generators import SMOLS, NeuralNetwork, SKLinearRegression
...
class RegressionBase(ipyw.VBox):
    ...
    def __init__(self, dataframe: pd.DataFrame):
        ...
        self.nn_button = ipyw.Button(
            layout=dict(width="40px", height="40px"),
            tooltip="Create Neural Network",
            icon="bolt",
        )
        self.nn_button.on_click(self.create_nn)

        self.lr_button = ipyw.Button(
            layout=dict(width="40px", height="40px"),
            tooltip="Create Linear Regression",
            icon="line-chart",
        )
        self.lr_button.on_click(self.create_lr)

        self.ols_button = ipyw.Button(
            layout=dict(width="40px", height="40px"),
            tooltip="Create OLS Regression",
            icon="plus-square-o",
        )
        self.ols_button.on_click(self.create_ols)

        # button to view the dataframe if desired
        self.view_dataframe_button = ipyw.Button(
            description="view dataframe",
            layout=dict(width="250px", height="40px"),
            tooltip="View Dataframe",
            icon="bars",
        )
    ...
    def _update_selectors(self, *_):
        ...
        if validation_value is not None:
            ...
            valid_input_values = all_columns - set(
                [
                    target_value,
                ]
            )
